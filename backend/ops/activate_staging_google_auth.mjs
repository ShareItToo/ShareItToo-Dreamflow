...
export async function runGoogleAuthActivation({ manifest, command = runCommand, commandEnv = process.env, execute = false } = {}) {
  ...
  try {
    ...
    const invalidToken = await executePhase(command, ['exec', target.apiContainer, 'node', '--input-type=module', '-e', "const r=await fetch('http://127.0.0.1:8080/v1/auth/social',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({idToken:'synthetic-invalid-token'})}); const p=await r.json(); process.stdout.write(JSON.stringify({status:r.status,error:p.error})); if(r.status!==401 || p.error!=='invalid_social_token') process.exit(1)"], { phase: 'replacement_invalid_social_token_probe', env: commandEnv });
    const invalidTokenPayload = parseJson(invalidToken.stdout, 'replacement_invalid_social_token_probe_invalid');
    if (invalidTokenPayload.status !== 401 || invalidTokenPayload.error !== 'invalid_social_token') fail('replacement_invalid_social_token_probe_invalid');
...
export async function runGoogleAuthIsolatedRehearsal({ manifest, command = runCommand, commandEnv = process.env } = {}) {
  ...
  try {
    ...
    const invalidToken = await executePhase(command, ['exec', candidate, 'node', '--input-type=module', '-e', "const r=await fetch('http://127.0.0.1:8080/v1/auth/social',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({idToken:'synthetic-invalid-token'})}); const p=await r.json(); process.stdout.write(JSON.stringify({status:r.status,error:p.error})); if(r.status!==401 || p.error!=='invalid_social_token') process.exit(1)"], { phase: 'rehearsal_invalid_social_token_probe', env: commandEnv });
    const invalidTokenPayload = parseJson(invalidToken.stdout, 'rehearsal_invalid_social_token_probe_invalid');
    if (invalidTokenPayload.status !== 401 || invalidTokenPayload.error !== 'invalid_social_token') fail('rehearsal_invalid_social_token_probe_invalid');
