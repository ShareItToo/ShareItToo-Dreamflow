...
if [[ "$task_environment" == production && "$task_enable_staging_mfa" == 1 ]]; then
  echo "The staging MFA override is forbidden for production deployments." >&2
  exit 1
fi
...
if [[ "$task_enable_staging_mfa" == 1 && "$task_staging_pilot_id" != heilbronn_wave0 ]]; then
  echo "The staging MFA override requires SIT_STAGING_PILOT_ID=heilbronn_wave0." >&2
  exit 1
fi
...
if [[ "$task_enable_staging_mfa" == 1 && "${CONFIRM_STAGING_MFA:-}" != "$task_commit" ]]; then
  echo "CONFIRM_STAGING_MFA must equal the exact deployment commit." >&2
  exit 1
fi
...
if [[ "$task_environment" == production ]]; then
  ...
  if [[ "$task_enable_staging_stripe" == 1 ]]; then
    ...
    PAYMENT_SANDBOX_AUTHORIZATION_ID="${PAYMENT_SANDBOX_AUTHORIZATION_ID:-}" \
    PAYMENT_SANDBOX_AUTH_ISSUED_AT="${PAYMENT_SANDBOX_AUTH_ISSUED_AT:-}" \
    PAYMENT_SANDBOX_AUTH_EXPIRES_AT="${PAYMENT_SANDBOX_AUTH_EXPIRES_AT:-}" \
      "$task_node_binary" "$task_backend_root/ops/validate_stripe_staging_execution_gate.mjs"
  ...
  if [[ "$task_enable_staging_mfa" == 1 ]]; then
    MFA_ENCRYPTION_KEY_HOST_FILE="${MFA_ENCRYPTION_KEY_HOST_FILE:-}" \
      "$task_node_binary" "$task_backend_root/ops/validate_mfa_staging_secret.mjs"
    task_compose_args+=(-f "$task_backend_root/compose.staging.mfa.yml")
    task_staging_mfa_enabled=true
  fi
fi
...
if [[ "$task_staging_mfa_enabled" == true ]] &&
   ! printf '%s' "$task_ready_payload" | "$task_node_binary" -e '
     const { readFileSync } = require("node:fs");
     const payload = JSON.parse(readFileSync(0, "utf8"));
     const boundary = payload?.checks?.mfa;
     const valid = boundary?.configured === true
       && boundary.credentialSource === "file";
     process.exitCode = valid ? 0 : 1;
   '; then
