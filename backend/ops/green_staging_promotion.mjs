...
export function assertGreenContainerInventory(inventory) {
  ...
  if (inventory.api.prePromotionTuple === true
      && (inventory.api.networks?.slice().sort().join('|') !== [greenTarget.network, greenTarget.providerNetwork].sort().join('|')
        ...
        || inventory.api.image !== inventory.api.expectedPrePromotionImage
...
export function buildGreenPromotionPlan({
  ...
  return Object.freeze({
    ...
    phases: Object.freeze([
      ...
      'explicitly migrate canonical Green 87 to 92, read back schema, and provision the synthetic user on Green',
...
export function buildGreenPromotionCommands({ plan, configFile, config } = {}) {
  ...
  const commands = [
    ...
    { phase: 'candidate_acceptance_create', command: 'docker', args: [
      ...
      '--env-file', configFile, '--env-file', isolated.envFile, '--publish', '127.0.0.1:18082:8080',
    ...
    { phase: 'candidate_live_wait', command: 'curl', args: ['--fail', '--silent', '--show-error', '--retry', '30', '--retry-delay', '1', '--retry-connrefused', 'http://127.0.0.1:18082/health/live'] },
    { phase: 'candidate_health_and_feature_probes', command: 'curl', args: ['--fail', '--silent', '--show-error', '--retry', '30', '--retry-delay', '1', '--retry-connrefused', 'http://127.0.0.1:18082/health'] },
    { phase: 'candidate_ready_probe', command: 'curl', args: ['--fail', '--silent', '--show-error', '--retry', '30', '--retry-delay', '1', '--retry-connrefused', 'http://127.0.0.1:18082/health/ready'] },
    { phase: 'candidate_version_probe', command: 'curl', args: ['--fail', '--silent', '--show-error', '--retry', '30', '--retry-delay', '1', '--retry-connrefused', 'http://127.0.0.1:18082/version'] },
    ...
    { phase: 'final_live_wait', command: 'curl', args: ['--fail', '--silent', '--show-error', '--retry', '30', '--retry-delay', '1', '--retry-connrefused', `${process.env.GREEN_STAGING_PUBLIC_BASE_URL ?? 'https://staging.shareittoo.com/api'}/health/live`] },
    { phase: 'final_health_probe', command: 'curl', args: ['--fail', '--silent', '--show-error', '--retry', '30', '--retry-delay', '1', '--retry-connrefused', `${process.env.GREEN_STAGING_PUBLIC_BASE_URL ?? 'https://staging.shareittoo.com/api'}/health`] },
    { phase: 'final_ready_wait', command: 'curl', args: ['--fail', '--silent', '--show-error', '--retry', '30', '--retry-delay', '1', '--retry-connrefused', `${process.env.GREEN_STAGING_PUBLIC_BASE_URL ?? 'https://staging.shareittoo.com/api'}/health/ready`] },
    { phase: 'final_version_readback', command: 'curl', args: ['--fail', '--silent', '--show-error', '--retry', '30', '--retry-delay', '1', '--retry-connrefused', `${process.env.GREEN_STAGING_PUBLIC_BASE_URL ?? 'https://staging.shareittoo.com/api'}/version`] },
  ];
...
export function assertGreenCommandBindings(commands, plan, configFile) {
  ...
  for (const phase of ['isolated_postgres_create', 'isolated_migrate_87_to_92']) {
  ...
  try {
    for (const entry of commands) {
      ...
      if (entry.phase.startsWith('target_inventory_') || entry.phase === 'sealed_name_conflict_check' || entry.phase === 'runtime_image_readback' || entry.phase === 'source_schema_readback'
          ...
          || entry.phase.endsWith('_schema_readback')) {
    ...
    const evidence = sanitizeGreenEvidence({ plan, status: 'executed', backupDigest, configDigest: sha256(await readFile(configFile)), targetReadback: { sourceSchema: greenTarget.sourceSchema, currentSchema: greenTarget.currentSchema, cleanup: 'verified', finalInventory: summarizeGreenFinalContainerReadback(JSON.parse(readbacks.final_inventory_readback), plan) }, imageReadback: JSON.parse(readbacks.final_version_readback) });
