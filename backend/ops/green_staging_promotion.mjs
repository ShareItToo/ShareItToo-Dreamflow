...
export function buildGreenPromotionCommands({ plan, configFile, config } = {}) {
  ...
  const commands = [
    ...
    { phase: 'fresh_protected_backup', command: 'docker', args: ['exec', target.databaseContainer, 'pg_dump', '--format=custom', '--no-owner', '--no-acl', '-U', 'shareittoo_staging', '-d', 'shareittoo_staging'], stdoutFile: `${plan.evidenceFile}.pgdump` },
...
export function runGreenCommand(command, args, { cwd = repositoryRoot, env = process.env, phase = 'green_command' } = {}) {
